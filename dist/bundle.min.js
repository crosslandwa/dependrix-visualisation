(function () {
	...
	*/
	...
	var updateArtifactFilter = function updateArtifactFilter(search) {
	  return {
	    type: 'UPDATE_ARTIFACT_FILTER',
	    search: sanitiseSearch(search)
	  };
	};
	var updateDependencyFilter = function updateDependencyFilter(search) {
	  return {
	...
	var artifactIds = function artifactIds(state) {
	  return filterBySearchTerms(Object.keys(state.artifacts), state.filters.artifacts).sort();
	};
	var artifactVersion = function artifactVersion(state, id) {
	  return state.artifacts[id].version;
	};
	var artifactDependencyVersion = function artifactDependencyVersion(state, id, dependencyId) {
	  return apply(state.artifacts[id].dependencies[dependencyId], function (dep) {
	    return dep && dep.version || '';
	  });
	};
	var artifactDependencyScope = function artifactDependencyScope(state, id, dependencyId) {
	  return apply(state.artifacts[id].dependencies[dependencyId], function (dep) {
	    return dep && dep.scope || '';
	  });
	};
	var isScopeAllowedByFilter = function isScopeAllowedByFilter(state, scope) {
	  return state.filters.scope.length ? state.filters.scope.includes(scope) : true;
	};
	var dependencyIds = function dependencyIds(state) {
	  return filterByScope(state, filterBySearchTerms(Object.keys(state.dependencies), state.filters.dependencies)).sort();
	};

	var filterBySearchTerms = function filterBySearchTerms(ids, search) {
	  return search.length ? ids.filter(function (id) {
	    return search.some(function (term) {
	      return id.toLowerCase().includes(term);
	    });
	  }) : ids;
	};

	var filterByScope = function filterByScope(state, ids) {
	  return apply(state.filters.scope, function (scope) {
	    return scope.length ? ids.filter(function (dependencyId) {
	      return artifactIds(state).some(function (id) {
	        return scope.includes(artifactDependencyScope(state, id, dependencyId));
	      });
	    }) : ids;
	  });
	};
	...
	var availableScopes = function availableScopes(state) {
	  return artifactIds(state).reduce(function (acc, id) {
	    return uniques(acc.concat(Object.keys(state.artifacts[id].dependencies).map(function (depId) {
	      return artifactDependencyScope(state, id, depId);
	    }))).filter(function (x) {
	      return x;
	    });
	  }, []);
	...
	var artifactsReducer = function artifactsReducer() {
	  var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	  var action = arguments.length > 1 ? arguments[1] : undefined;
	...
	var dependenciesReducer = function dependenciesReducer() {
	  var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	  var action = arguments.length > 1 ? arguments[1] : undefined;
	...
	var filtersReducer = function filtersReducer() {
	  ...
	  switch (action.type) {
	    case 'UPDATE_ARTIFACT_FILTER':
	      return _objectSpread$1({}, state, {
	        artifacts: action.search
	      });

	    case 'UPDATE_DEPENDENCY_FILTER':
	      return _objectSpread$1({}, state, {
	        dependencies: action.search
	      });
	...
	var Initialising = function Initialising() {
	  return react.createElement("div", null, "Initialising...");
	};

	var LoadFailed = function LoadFailed() {
	  return react.createElement("div", null, "Tree load failed");
	};
	  ...
	  _createClass(LoadSummary, [{
	    ...
	    value: function render() {
	      var _this$props = this.props,
	          loadAttempted = _this$props.loadAttempted,
	          loadSuccess = _this$props.loadSuccess;
	      return loadAttempted ? loadSuccess ? null : react.createElement(LoadFailed, null) : react.createElement(Initialising, null);
	    }
	  }]);
	...
	var apply$1 = function apply(x, f) {
	  return f(x);
	};

	var mapStateToProps$1 = function mapStateToProps(state, _ref) {
	  var artifactId = _ref.artifactId,
	      dependencyId = _ref.dependencyId;
	  return apply$1(apply$1(artifactDependencyVersion(state, artifactId, dependencyId), function (version) {
	    return {
	      version: version,
	    ...
	    var version = _ref2.version,
	        scope = _ref2.scope;
	    return scope && isScopeAllowedByFilter(state, scope) ? {
	      version: version,
	      scope: scope
	    } : {};
	  });
	...
	var DependencyCell = function DependencyCell(_ref3) {
	  var scope = _ref3.scope,
	      version = _ref3.version;
	  return version ? react.createElement("td", {
	    "class": "matrix__table-cell"
	  }, version, react.createElement("br", null), scope) : react.createElement("td", {
	    "class": "matrix__table-cell"
	  });
	};

	var DependencyCell$1 = connect(mapStateToProps$1)(DependencyCell);

	var apply$2 = function apply(x, f) {
	  return f(x);
	};

	var mapStateToProps$2 = function mapStateToProps(state) {
	  return apply$2(artifactIds(state), function (ids) {
	    return {
	      ids: ids,
	      versions: ids.reduce(function (acc, id) {
	        return _objectSpread$1({}, acc, _defineProperty$1({}, id, artifactVersion(state, id)));
	      }, {}),
	...
	var DependencyMatrix = function DependencyMatrix(_ref) {
	  var ids = _ref.ids,
	      ...
	      dependencyIds = _ref.dependencyIds,
	      loaded = _ref.loaded;
	  ...
	  }))), react.createElement("tbody", null, dependencyIds.map(function (dependencyId) {
	    return react.createElement("tr", null, react.createElement("td", {
	      "class": "matrix__table-cell matrix__table-cell--lh-column"
	    }, dependencyId), ids.map(function (id) {
	      return react.createElement(DependencyCell$1, {
	        artifactId: id,
	...
	var mapDispatchToProps = function mapDispatchToProps(dispatch) {
	  return {
	    update: debouncedEventValue(500, function (value) {
	      return dispatch(updateArtifactFilter(value));
	    })
	  };
	};

	var ArtifactFilter = function ArtifactFilter(_ref) {
	  var update = _ref.update;
	  return react.createElement(react.Fragment, null, react.createElement("label", {
	    "class": "input-label",
	    "for": "artifactFilter"
	  }, "Filter artifacts"), react.createElement("input", {
	    id: "artifactFilter",
	    type: "text",
	    "class": "input-text",
	...
	var ArtifactFilter$1 = connect(undefined, mapDispatchToProps)(ArtifactFilter);

	var mapDispatchToProps$1 = function mapDispatchToProps(dispatch) {
	  return {
	    update: debouncedEventValue(500, function (value) {
	      return dispatch(updateDependencyFilter(value));
	    })
	  };
	};

	var DependencyFilter = function DependencyFilter(_ref) {
	  var update = _ref.update;
	  return react.createElement(react.Fragment, null, react.createElement("label", {
	    "class": "input-label",
	    "for": "artifactFilter"
	  }, "Filter dependencies"), react.createElement("input", {
	    id: "artifactFilter",
	    type: "text",
	    "class": "input-text",
	...
	var DependencyFilter$1 = connect(undefined, mapDispatchToProps$1)(DependencyFilter);
	...
	function (_React$Component) {
	  ...
	  _createClass(ScopeFilter, [{
	    ...
	    value: function render() {
	      ...
	      return react.createElement("fieldset", null, react.createElement("legend", null, "Filter by scope"), react.createElement("div", null, availableScopes.map(function (scope) {
	...
	var App = function App(props) {
	  return react.createElement(react.Fragment, null, react.createElement("h1", {
	    "class": "header--xl"
	  }, "Dependrix"), react.createElement("details", {
	    "class": "details",
	    open: true
	  ...
	  }, react.createElement(ArtifactFilter$1, null), react.createElement(DependencyFilter$1, null), react.createElement(ScopeFilter$1, null))), react.createElement(LoadSummary$1, null), react.createElement(DependencyMatrix$1, null));
	};

	var App$1 = connect(mapStateToProps$4, mapDispatchToProps$3)(App);
